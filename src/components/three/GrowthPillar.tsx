...
/** Deterministic "grown" layer table — the result of building upwards in layers. */
...
/** Small luminous core windowed inside the transparent top of the pillar. */
...
/**
 * GROWTH PILLAR — a monumental, layered graphite tower.
 *
 * Geometry is procedural: a stack of beveled slabs (merged into one BufferGeometry
 * for a single draw call), thin emissive "growth rings" at every interface, slim
 * accent lines climbing the corners, a quiet row of data nodes and a restrained
 * transparent inner core. — Growth, built in layers.
 */
export function GrowthPillar() {
  ...
  // merged geometry for the opaque tower (one draw call)
  ...
  // transparent cap showing the inner core
  ...
  const nodeMat = useMemo(
    () => new THREE.MeshBasicMaterial({ color: '#c8ff3d', transparent: true, opacity: 0.85 }),
    [],
  )

  // small emissive data terminals mounted at slab corners
  const nodes = useMemo(() => {
  ...
  useFrame((state) => {
    ...
    // entrance: the tower drops in and settles + extremely subtle idle sway
  ...
  return (
    <group ref={origin}>
      ...
      {/* transparent cap revealing the inner core */}
      <mesh geometry={capGeometry} material={glassMat} />

      {/* emissive growth rings at every interface */}
      {layers.map((l, i) => (
        <RoundedBox
          key={`ring-${i}`}
      ...
      {/* thin accent lines climbing the tower */}
